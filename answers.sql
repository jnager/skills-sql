...
SELECT models.name, models.brand_name, brands.headquarters FROM brands INNER JOIN models ON (models.brand_name=brands.name) WHERE models.name ='Mustang';
...
SELECT m.brand_name FROM brands AS b LEFT JOIN models AS m ON (b.name = m.brand_name) GROUP BY m.brand_name HAVING COUNT(*) > 5;
